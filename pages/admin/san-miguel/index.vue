...
async function load() {
  ...
  try {
    const [countRes, listRes] = await Promise.all([
      ...
      supabase
          .from("web_inquiries")
          .select("id, created_at, name, business, email, message, page, locale")
    ...
    err.value = e?.message || "Failed to load inquiries."
  } finally {
    loading.value = false
  }
}
...
<template>
  <div>
    <div class="flex items-end justify-between gap-4">
      <div>
        <h1 class="text-2xl font-semibold tracking-tight">San Miguel inquiries</h1>
